...
public class workpolicyTC extends commonfunctions {
	...
	public void eventlist() throws Exception {
		...
		if (actCountry.equals("WorkPolicy")) {
			...
			test.fail("<font color='red'>MissPuchPOlicy page is not displayed</font>",
		...
		if (workpolicyObj.MisspuchHead.getText().equals("Miss Punch Policy")) {
			...
			test.fail("<font color='red'>Create Workpolicy page navigation failed</font>",
					MediaEntityBuilder.createScreenCaptureFromPath(takeScreenshot(driver, screenShot)).build());
		}
		...
		for (int i = 0; i < workpolicyObj.DeductionList.size(); i++) {
			if (workpolicyObj.DeductionList.get(i).getText()
					.equalsIgnoreCase(properties.getProperty("DeductionType1"))) {
		...
		if (workpolicyObj.assignHeadmiss.getText().equals("Assign Policy")) {
			...
			test.fail("<font color='red'>Assign policy page navigation failed</font>",
					MediaEntityBuilder.createScreenCaptureFromPath(takeScreenshot(driver, screenShot)).build());
		}
		...
		workpolicyObj.DateSelect.click();
		test.pass("Effective date clicked successfully");
		Thread.sleep(2000);
		...
		if (workpolicyObj.DashSerch.getText().contains(properties.getProperty("PolicyName"))) {
			...
			test.fail("<font color='red'>Policy not found in the search results</font>",
					MediaEntityBuilder.createScreenCaptureFromPath(takeScreenshot(driver, screenShot)).build());
		}
	...
	public void overtime() throws Exception {
		...
		if (actCountry.equals("WorkPolicy")) {
			...
			test.fail("<font color='red'>WorkPolicy page  page is not displayed</font>",
					MediaEntityBuilder.createScreenCaptureFromPath(takeScreenshot(driver, screenShot)).build());
		...
		if (OvertimeName.equals("Over Time Policy")) {
			...
			test.fail("<font color='red'> Over Time Policy page is not displayed</font>",
					MediaEntityBuilder.createScreenCaptureFromPath(takeScreenshot(driver, screenShot)).build());
		}
		Thread.sleep(3000);
	...
	public void timeout() throws Exception {
		...
		if (Timeout.equals("Time In-Out Policy")) {
			...
			test.fail("<font color='red'> Time In-Out Policy page is not displayed</font>",
					MediaEntityBuilder.createScreenCaptureFromPath(takeScreenshot(driver, screenShot)).build());
		}
		...
		// UPDATE POLICY
		test.info(
				"<b><font color = 'purple'>Test case-4  Verify the Update Time In Time Out Policy functionality  </b>");
	...
	public void Offdays() throws Exception {
		...
		if (Timeout.equals("Attendance on Off days")) {
			...
			test.fail("<font color='red'>Attendance on Off days page is not displayed</font>",
					MediaEntityBuilder.createScreenCaptureFromPath(takeScreenshot(driver, screenShot)).build());
		}
		...
		for (int i = 0; i < workpolicyObj.MultiplierDD.size(); i++) {
			if (workpolicyObj.DeductionList.get(i).getText()
					.equalsIgnoreCase(properties.getProperty("SalaryMultiplier"))) {
				workpolicyObj.DeductionList.get(i).click();
				break;
			}
		}
		...
		for (int i = 0; i < workpolicyObj.ComponentDD.size(); i++) {
			if (workpolicyObj.ComponentDD.get(i).getText()
					.equalsIgnoreCase(properties.getProperty("SalaryComponent"))) {
		...
		test.pass("No button clicked successfully");
		Thread.sleep(2000);
		workpolicyObj.DashSerch.sendKeys(properties.getProperty("Offdaypolicy"));
		Thread.sleep(4000);
		workpolicyObj.DeletBtnmis.click();
		Thread.sleep(1000);
		...
		// DAshborad
		// Loop through and click each option one by one, then close and proceed to the
		// next
		// Loop through and click each option one by one, then close and proceed to the
		// next

		test.info("<b><font color = 'purple'>Test case-6  Verify the Dashboard options and their counts </b>");
		WebElement[] dashboardOptions = {
				workpolicyObj.IOPolicyName,
				workpolicyObj.OverTimepolicyName,
				workpolicyObj.AOFPolicyName,
				workpolicyObj.MissPolicyName
		};

		String[] optionNames = {
				"Time In-Out Policy",
				"Overtime Policy",
				"Attendance on Off days",
				"Miss Punch Policy"
		};

		// Each option has its own count element, assumed as below. Replace with actual
		// locators if different.
		WebElement[] optionCounts = {
				workpolicyObj.TimeInOutCount,
				workpolicyObj.OvertimeCount,
				workpolicyObj.AttendOffdaysCount,
				workpolicyObj.MissPunchCount
		};
		for (int i = 0; i < dashboardOptions.length; i++) {
			dashboardOptions[i].click();
			test.pass("<b>" + optionNames[i] + "</b> option clicked successfully");
			Thread.sleep(1000);
			String countText = optionCounts[i].getText();
			test.info("<b>" + optionNames[i] + "</b> count: <b>" + countText + "</b>");
			// Close the option window if needed
			workpolicyObj.CloseOption.get(0).click();
			Thread.sleep(1000);
		}

		test.pass("All dashboard options clicked and counts displayed successfully");
	}
